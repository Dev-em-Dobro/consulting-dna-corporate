...
const cases = [
  {
    client: "Heineken",
    sector: "FMCG",
    challenge: "Accelerate the readiness and advancement of high-potential leaders across the group.",
    approach: "A cohort programme pairing DNA 360 diagnostics with 5H coaching and sponsor-backed stretch assignments.",
    metric: "45%",
    metricLabel: "higher promotion rate for programme participants",
    evidence: "Promotion tracking across two cohorts vs. a matched control group.",
  },
  {
    client: "Frasers Property",
    sector: "Real estate",
    challenge: "Retain critical leadership talent through a period of strategic change.",
    approach: "Executive-team alignment work plus individual coaching for the leaders most exposed to the transition.",
    metric: "85%",
    metricLabel: "talent retention among participating leaders",
    evidence: "12-month retention of the identified critical-leader population.",
  },
  {
    client: "Shell",
    sector: "Energy",
    challenge: "Scale women's leadership development across a global engineering workforce.",
    approach: "A globally consistent curriculum delivered locally by regional faculty across multiple markets.",
    metric: "2,582",
    metricLabel: "women leaders impacted across the programme",
    evidence: "Aggregate participation across the multi-year global rollout.",
  },
];

const people = [
  { name: "Rhea", role: "Founder & Senior Partner", region: "Global" },
  { name: "Regional Partner", role: "Executive Coaching & Advisory", region: "London" },
  { name: "Regional Partner", role: "Executive Coaching & Advisory", region: "Singapore" },
  { name: "Regional Partner", role: "Executive Coaching & Advisory", region: "Dubai · Riyadh" },
];

const insights = [
  {
    tag: "Article",
    title: "Why executive teams stall — and how alignment unlocks speed.",
    body: "The gap between a good strategy and a fast organisation is almost always a leadership-alignment gap. Here is where it hides.",
    meta: "6 min read",
  },
  {
    tag: "Framework",
    title: "The inner and outer game of leadership under pressure.",
    body: "How the 5H methodology works on identity and habits, not just skills — and why that is what makes change hold.",
    meta: "5 min read",
  },
  {
    tag: "Research",
    title: "What separates leaders who are ready from leaders who look ready.",
    body: "Findings from DNA 360 diagnostics on the difference between visible confidence and genuine enterprise readiness.",
    meta: "8 min read",
  },
];
    ...
    <div className="w-full overflow-x-hidden bg-white">
            ...
            {cases.map((c) => (
              <article key={c.client} className="flex flex-col border border-line">
                ...
                <div className="flex flex-1 flex-col px-[26px] py-7">
                  ...
                  <p className="mb-[18px] text-[15px] leading-[1.55] text-[#4a4548]">{c.challenge}</p>
                  <p className="mb-1.5 text-[11px] font-bold uppercase tracking-[1.5px] text-brand">Approach</p>
                  <p className="mb-[22px] text-[15px] leading-[1.55] text-[#4a4548]">{c.approach}</p>
                  <div className="mt-auto border-t border-line pt-[22px]">
                    <p className="mb-1.5 text-[11px] font-bold uppercase tracking-[1.5px] text-brand">Result</p>
                    ...
                    <p className="mt-4 border-t border-line pt-3.5 text-[12.5px] leading-[1.5] text-muted">
                      <span className="font-semibold uppercase tracking-[0.5px] text-muted">Evidence:</span> {c.evidence}
                    </p>
                  </div>
                </div>
              </article>
      ...
      <section id="people" className="bg-white">
        <Reveal className="mx-auto max-w-[1200px] px-10 py-24">
          ...
          <p className="mb-12 max-w-[640px] text-lg leading-[1.55] text-muted">
            Led from London, Singapore, Dubai and Riyadh — a leadership team of seasoned advisors, backed by a global faculty of 75 practitioners delivering across 36 countries.
          </p>
            ...
            {people.map((p, i) => (
              <div key={i}>
                <div className="relative aspect-[3/4] overflow-hidden bg-[#e9e6e3]">
                  ...
                  <span className="absolute left-3 top-3 bg-ink/85 px-2.5 py-1 text-[10.5px] font-semibold uppercase tracking-[1px] text-white">
                    {p.region}
                  </span>
      ...
      <section id="insights" className="bg-paper">
        <Reveal className="mx-auto max-w-[1200px] px-10 py-24">
          <div className="mb-2.5 flex items-baseline gap-3">
            <span className="inline-block h-0.5 w-9 bg-brand" />
            <span className="text-[13px] font-semibold uppercase tracking-[2px] text-brand">Insights</span>
          </div>
          <h2 className="mb-3 max-w-[720px] text-[30px] sm:text-[34px] md:text-[40px] font-bold leading-[1.1] tracking-[-0.8px] text-ink">
            Thinking from the front line of enterprise leadership.
          </h2>
          <p className="mb-[52px] max-w-[620px] text-lg leading-[1.55] text-muted">
            Perspectives, frameworks and research on what makes leadership hold when the stakes are highest.
          </p>

          <div className="mb-16 grid grid-cols-1 gap-px border border-line bg-line md:grid-cols-3">
            {insights.map((post) => (
              <a key={post.title} href="#contact" className="group flex flex-col bg-white p-8 hover:bg-[#fafafa]">
                <span className="mb-5 self-start bg-brand/10 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[1px] text-brand">
                  {post.tag}
                </span>
                <h3 className="mb-3 text-[20px] font-semibold leading-snug tracking-[-0.3px] text-ink">{post.title}</h3>
                <p className="mb-8 text-[15px] leading-[1.55] text-[#4a4548]">{post.body}</p>
                <div className="mt-auto flex items-center justify-between border-t border-line pt-4">
                  <span className="text-[12.5px] font-medium text-muted">{post.meta}</span>
                  <span className="text-sm font-bold text-brand transition-transform group-hover:translate-x-1">→</span>
                </div>
              </a>
            ))}
          </div>
