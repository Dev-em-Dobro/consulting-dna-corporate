 ...
 * `CDNA_04_Team.docx` pede, para o bloco 2, "portrait grid, three across.
 * Name, role, region, short bio on click or hover", e o botão "+" do mockup é
 * esse gesto. Copiar o modal para a /team criaria dois perfis de pessoa no
 * mesmo site, que divergem na primeira correção feita de um lado só.
 *
 * QUEM USA: a `/team` (pelo `LeaderCard`) e o `PeopleGrid`, que continua
...
export function Avatar({ person, sizes }: { person: Person; sizes: string }) {
  if (person.img) {
    return (
      <Image
        src={person.img}
...
export default function PersonModal({ person, onClose }: { person: Person; onClose: () => void }) {
  useEffect(() => {
